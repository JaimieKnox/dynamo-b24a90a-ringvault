...
Every gate seal (request, continue, hold, claim) requires a gate_tag appended to the seal material. The gate_tag is 8 raw binary bytes (not hex-encoded) that bind the seal to a specific schedule position and gate type. The gate_tag is not present in case.json and cannot be obtained by enumerating gate types or restarting the binary.

The gate_tag for a given gate is the first 8 bytes of SHA-256(engine-internal secret bytes || challenge_id || "|" || script_pos || "|" || event_code), used as raw bytes (not hex-encoded into material). Here script_pos is the zero-based decimal index of the gate in the decoded schedule, and event_code is the decimal numeric code for the gate type at that position. The engine-internal secret is the same binary-only salt that protects ticket issuance and schedule decryption. It is not disclosed in case.json or this brief.
...
The request seal tag is the SHA-256 digest of these bytes in order: the 32 raw key bytes, one ASCII pipe separator, then the material bytes. The material begins with the ASCII portions slot, scope, and the literal word request joined by ASCII pipe separators, followed by a final ASCII pipe, then the 8 raw gate_tag bytes. Example shape: key_bytes + "|" + "slot|scope|request|" + gate_tag_raw_8 with decimal slot and no extra spaces.
...
Some sessions include hold gates. A hold gate is not present in the disclosed fit packs. When the schedule reaches a hold gate the binary enters a hold phase. The caller must send a frame that contains a HOLD_SEAL TLV (type 0x08) with a 32-byte hold seal tag. The hold seal tag is the SHA-256 digest of these bytes in order: the 32 raw key bytes, one ASCII pipe separator, then the material bytes. The hold material begins with the literal ASCII "hold" followed by a pipe and the lowercase hex encoding of the live ticket bytes (or a literal dash "-" if no ticket is currently live), followed by a pipe, then the 8 raw gate_tag bytes. Example shapes: key_bytes + "|" + "hold|ticket_hex|" + gate_tag_raw_8 when a live ticket exists, or key_bytes + "|" + "hold|-|" + gate_tag_raw_8 when no live ticket exists. On success the binary replies with NOTE value "held" and then advances the schedule. A missing, wrong-type, or wrong-value hold frame permanently locks the session and returns a decoy. Work sessions may use hold composition that is not present in fit packs. Graded work schedules are not listed here.
...
The continue seal tag is the SHA-256 digest of these bytes in order: the 32 raw key bytes, one ASCII pipe separator, then the material bytes. The continue material begins with the literal ASCII "cont" followed by a pipe, the lowercase hex encoding of the live ticket bytes, another pipe, then the 8 raw gate_tag bytes. The continue gate_tag uses the same script_pos and event_code as the parent ticket gate. Example shape: key_bytes + "|" + "cont|ticket_hex|" + gate_tag_raw_8.
...
The claim seal tag is the SHA-256 digest of these bytes in order: the 32 raw key bytes, one ASCII pipe separator, then the material bytes. The material begins with the ASCII portions slot, scope, and ticket_hex (lowercase hex encoding of the live ticket bytes) joined by ASCII pipe separators, followed by a final ASCII pipe, then the 8 raw gate_tag bytes. Example shape: key_bytes + "|" + "slot|scope|ticket_hex|" + gate_tag_raw_8 with decimal slot and no extra spaces.
