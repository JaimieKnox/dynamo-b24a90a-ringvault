...
Drive the challenge binary at /app/data/bin/vaultlab for each work case. The binary speaks a length-prefixed frame protocol on stdin/stdout and has no inspectable internal state. Parse all session state from reply frames. Disclosed fit packs are under /app/data/fit/. Induce framing, ticket issue, ticket invalidation after reincarnate, seal-tag construction, nesting, and generation-binding behavior from the brief and fit-pack probing. Compare REPLY values against each case decoy_flag to distinguish success from failure when probing fit packs. Write /app/output/flags.json for the work set only.
